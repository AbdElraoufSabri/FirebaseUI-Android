...
public class EmailActivity extends AppCompatBase implements CheckEmailFragment.CheckEmailListener {
    ...
    @Override
    public void onExistingEmailUser(@NotNull User user, @NotNull String password) {
            startActivityForResult(
                    WelcomeBackPasswordPrompt.Companion.createIntent(this, getFlowParams(), new IdpResponse.Builder(user).build()),RequestCodes.WELCOME_BACK_EMAIL_FLOW);
            setSlideAnimation();
    ...
    public void onNewUser(User user) {
        ...
        if (emailConfig.getParams().getBoolean(ExtraConstants.ALLOW_NEW_EMAILS, true)) {
            FragmentTransaction ft = getSupportFragmentManager().beginTransaction();
                RegisterEmailFragment fragment = RegisterEmailFragment.Companion.newInstance(user);
                ft.replace(R.id.fragment_register_email, fragment,
                        RegisterEmailFragment.Companion.getTAG());
                if (emailLayout != null) {
